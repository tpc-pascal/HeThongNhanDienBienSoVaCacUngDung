...
// Image validation for React Native (using asset info)
export function validateImageFile(uri: string, size?: number): ValidationResult {
  if (!uri || typeof uri !== 'string') {
    return { isValid: false, error: 'URI ảnh không hợp lệ' };
  }
  ...
  return { isValid: true, sanitizedValue: uri };
}
